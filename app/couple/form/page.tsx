...
export default function CoupleForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const sessionId = `couple_${Date.now()}`
    ...
    router.push(`/couple/display?session=${sessionId}`)
  }
            ...
            >
              <div>
                ...
                <textarea
                  ...
                  rows={5}
                  className="w-full px-4 py-3 rounded-lg border-2 border-gray-300 focus:border-pink-500 focus:outline-none transition"
                  placeholder="Write a personal message for the New Year..."
                />
              </div>
            </motion.div>
          )}
            ...
            <div className="ml-auto">
              {currentStep < totalSteps ? (
                <button
                  type="button"
                  onClick={handleNext}
                  className="px-6 py-3 bg-pink-500 text-white rounded-lg font-semibold hover:bg-pink-600 transition flex items-center gap-2"
                >
